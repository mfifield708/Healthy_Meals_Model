...
# Ensure scikit-learn version matches the one used to save the model
# This is crucial for successful model loading and prediction.
!pip install scikit-learn==1.7.2 --quiet
...
try:
    ...
    # In this fallback, the OneHotEncoder is not directly used for prediction features as per current predict function logic.
    # Therefore, we remove its setup here to avoid confusion and potential inconsistencies if not explicitly used.
    ...
    # Corrected dummy feature_cols to match the structure of the real model's features (from kernel state)
    # These include engineered numerical features and target-encoded categorical features (TE_ prefixed).
    feature_cols = [
    ...
    # To address NotFittedError, fit the dummy model with dummy data
    n_samples = 10 # Number of dummy samples
    X_dummy = pd.DataFrame(np.random.rand(n_samples, len(feature_cols)), columns=feature_cols)
    y_dummy = np.random.randint(0, 2, n_samples) # Dummy target variable (0 or 1)
    model.fit(X_dummy, y_dummy) # Fit the dummy model

    # encoder.fit(sample_data) # This line is removed as OneHotEncoder is not used for prediction in this path.
...
def predict(
    ...
    for col in feature_cols:
        if col in processed_df.columns:
            ...
            # Handle cases where a feature might be missing or not directly derivable from inputs
            # This case should ideally not happen if feature_cols are correctly aligned with engineering steps.
            final_features_df[col] = 0.0 # Assign a default if feature is missing
...
with st.form("prediction_form"): # Using a form for better input management
    ...
    with col1:
        ...
        income_level = st.radio("Income Level", ["Low", "Medium", "High", "Very High"], index=1) # Default to 'Medium'
    ...
    with col2:
        ...
        education = st.radio("Education", ["High School", "Other", "Graduate", "Post-Graduate"], index=2) # Default to 'Graduate'

    with col3:
        device_type = st.radio("Device Type", ["Multi-device", "Mobile-only", "Desktop-only"], index=0) # Default to 'Multi-device'

    st.markdown("--- ") # Separator
    submitted = st.form_submit_button("Predict Renewal Probability")

if submitted:
    # Call the predict function with gathered inputs
